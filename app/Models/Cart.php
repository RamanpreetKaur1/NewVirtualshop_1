...
class Cart extends Model
{
    ...
    public static function userCartItems()
    {
        if (Auth::check()) {
            $userCartItems = Cart::with(['product' => function($query){
                $query->select('id' , 'product_name' , 'product_code' , 'product_image' , 'product_color');
    ...
    {
        $attrPrice = ProductsAttribute::select('original_price')->where(['product_id' => $product_id , 'size' => $size])->first()->toArray();
        return $attrPrice['original_price'];
    }
}
